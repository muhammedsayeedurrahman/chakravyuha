...
class GuidedFlow:
    ...
    def _canonical_node_id(node_id: str | None) -> str:
        """Map the legacy HTTP root identifier to the tree's actual root."""
        return "start" if not node_id or node_id == "root" else node_id
    ...
    def get_current_question(self, state: dict) -> dict:
        ...
        """
        node_id = self._canonical_node_id(state.get("current_node"))
    ...
    def process_answer(self, state: dict, answer_index: int) -> dict:
        ...
        new_state = {
            **state,
            ...
            "history": state.get("history", []) + [node_id],
        }
